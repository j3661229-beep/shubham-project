...
const seed = async () => {
  try {
    ...
    const catFurniture = await Category.create({ name: 'Furniture', description: 'Home and office furniture' });
    const catFood = await Category.create({ name: 'Food & Beverage', description: 'Perishable and non-perishable goods' });
    ...
    const wh3 = await Warehouse.create({ name: 'European Logistics Center', location: 'Berlin, Germany', capacity: 60000 });
    const wh4 = await Warehouse.create({ name: 'Asia Pacific Hub', location: 'Singapore', capacity: 45000 });
    ...
    const supLogi = await Supplier.create({ name: 'LogiParts Inc.', contact_email: 'sales@logiparts.com', rating: 4.2 });

    const custAlpha = await Customer.create({ name: 'Alpha Retailers', email: 'procurement@alpharetail.com' });
    const custBeta = await Customer.create({ name: 'Beta Solutions', email: 'support@betasolutions.com' });
    ...
    const prodDesk = await Product.create({ name: 'Ergonomic Desk', category_id: catFurniture.id, price: 599.00 });
    const prodMonitor = await Product.create({ name: '4K UltraWide Monitor', category_id: catElectronics.id, price: 799.99 });
    ...
    await Inventory.bulkCreate([
      ...
      { product_id: prodDesk.id, warehouse_id: wh3.id, quantity: 150 },
      { product_id: prodMonitor.id, warehouse_id: wh4.id, quantity: 300 },
      // low stock examples
      { product_id: prodLaptop.id, warehouse_id: wh2.id, quantity: 15 },
      { product_id: prodDesk.id, warehouse_id: wh1.id, quantity: 5 }
    ]);
